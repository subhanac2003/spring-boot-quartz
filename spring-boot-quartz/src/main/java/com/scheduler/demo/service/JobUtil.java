...
import com.scheduler.demo.config.PersistableCronTriggerFactoryBean;
...
class JobUtil {
	...
	protected static JobDetail createJob(Class<? extends QuartzJobBean> jobClass, boolean isDurable, 
	    ...
	    return createJob(jobClass, isDurable, 
				context, jobName, jobGroup , null);
	}
	
	protected static JobDetail createJob(Class<? extends QuartzJobBean> jobClass, boolean isDurable, 
			ApplicationContext context, String jobName, String jobGroup, String instance_id){
        ...
        if(instance_id != null && !instance_id.isEmpty() ) {
        	jobDataMap.put("instance_id", instance_id);	
        }
